...
@pytest.mark.usefixtures("api_gw_json_data")
def test_v1_check_proxy_with_right_format(api_gw_json_data):
    api_gw_validator_instance = create_api_gw_validator("v1")
    proxy = api_gw_json_data["routeSpecification"][0]
    result = api_gw_validator_instance.check_proxy(proxy)
    assert result is True


@pytest.mark.usefixtures("api_gw_wrong_json_data")
def test_v1_check_proxy_with_wrong_format(api_gw_wrong_json_data):
    api_gw_validator_instance = create_api_gw_validator("v1")
    proxy = api_gw_wrong_json_data["routeSpecification"][0]
    result = api_gw_validator_instance.check_proxy(proxy)
    assert result is False
